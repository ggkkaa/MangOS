...
void map_kernel() {
  ...
  kinfo("Mapping read-only part of the kernel. Physical address %p to virtual address %p, %d pages.",
  ...
  kinfo("Mapping writeable part of the kernel. Physical address %p to virtual address %p, %d pages.",
        phys, page_align_down((uintptr_t)WRITE_ALLOWED_START), length);

  page_mmap(kernel.pml4, phys, page_align_down((uintptr_t)WRITE_ALLOWED_START), length,
...
bool page_mmap(page_t pml4_address, paddr_t physical_addr,
  ...
  uint16_t pml2 = (virtual_addr >> 21) & 0x1ff;
  uint16_t pml3 = (virtual_addr >> 30) & 0x1ff;
  uint16_t pml4 = (virtual_addr >> 39) & 0x1ff;
